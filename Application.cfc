...
<cfcomponent output="false">
    ...
    <!---
        No # characters anywhere in this function — Lucee parses # in ALL
        string literals (both tag and cfscript, single or double quoted).
        All colors use rgb() notation instead of hex.
    --->
    <cffunction name="onError" returntype="void" output="false">
        ...
        <cfscript>
            ...
            // All colors use rgb() — no # characters anywhere.
            // bg-primary=#0d1117  bg-card=#1c2128  border=#30363d
            // text=#e6edf3  muted=#6e7681  red=#f85149  blue=#388bfd
            // cyan=#79c0ff  mid-gray=#8b949e
            local.css = "body{background:rgb(13,17,23);color:rgb(230,237,243);font-family:sans-serif;padding:2rem;}"
                ...
                & ".lb{font-size:.7rem;text-transform:uppercase;letter-spacing:.08em;color:rgb(110,118,129);font-family:monospace;margin-bottom:.2rem;}"
                & ".vl{background:rgb(13,17,23);border:1px solid rgb(48,54,61);border-radius:4px;padding:.5rem .8rem;font-family:monospace;font-size:.82rem;white-space:pre-wrap;word-break:break-all;color:rgb(230,237,243);max-height:280px;overflow-y:auto;margin-bottom:1rem;}"
            ...
            if (len(local.hint)) {
                local.html &= "<div class=""ht"">" & local.hint & "</div>";
            }

            if (local.isAdmin) {
                local.html &= "<div class=""lb"">Event</div><div class=""vl"">"   & htmlEditFormat(arguments.eventName)         & "</div>";
                local.html &= "<div class=""lb"">Message</div><div class=""vl"">" & htmlEditFormat(arguments.exception.message) & "</div>";

                if (len(trim(arguments.exception.detail))) {
                    local.html &= "<div class=""lb"">Detail</div><div class=""vl"">" & htmlEditFormat(arguments.exception.detail) & "</div>";
                }
                if (structKeyExists(arguments.exception, "queryError") AND len(trim(arguments.exception.queryError))) {
                    local.html &= "<div class=""lb"">Query Error</div><div class=""vl"">" & htmlEditFormat(arguments.exception.queryError) & "</div>";
                }
                if (structKeyExists(arguments.exception, "sql") AND len(trim(arguments.exception.sql))) {
                    local.html &= "<div class=""lb"">SQL</div><div class=""vl"">" & htmlEditFormat(arguments.exception.sql) & "</div>";
                }
                if (structKeyExists(arguments.exception, "stackTrace") AND len(trim(arguments.exception.stackTrace))) {
                    local.html &= "<div class=""lb"">Stack Trace</div><div class=""vl"">" & htmlEditFormat(arguments.exception.stackTrace) & "</div>";
                }
            } else {
